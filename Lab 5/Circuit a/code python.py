...
for i in range(10):
    ...
    incertitude_puiss= P*math.sqrt((inc_res_1[i]/res_1[i])**2+(inc_tens_1[i]/tens_1[i])**2)
...
for i in range(10):
    ...
    incertitude_puiss= P*math.sqrt((inc_res_2[i]/res_2[i])**2+(inc_tens_2[i]/tens_2[i])**2)
    print(P,",",incertitude_puiss)  
...
#on trace la fonction avec ces paramètres
def func_fitted2(x):
    return (x*b_fit2**2)/((c_fit2+x)**2+d_fit2**2)*1000
x = np.linspace(10**0,10**3,1000)
...
ax.plot(x, func_fitted2(x), 'g-', label='Courbe théorique avec le condensateur')
